...
png("Rplot4.png") # create ping file
...
# Chart Global Active Power as a function of time
with(power2, plot(datetime, kitchen, type = "n",
                  xlab = "Day",
                  ylab = "Energy sub-metering"))
legend("topright",
       legend = c("kitchen","laundry room","AC/water heater"),
       col = c("black", "blue", "red"),
       lty = "solid",
       cex = 0.75)
with(power2, lines(datetime, kitchen))
with(power2, lines(datetime, laundry, col = "blue"))
with(power2, lines(datetime, ac, col = "red"))

# Chart Voltage as a function of datetime
with(power2, plot(datetime, volts, type = "n"))
with(power2, lines(datetime, volts))

# Chart Reactive power as a function of datetime
with(power2, plot(datetime, react_pwr, type = "n"))
with(power2, lines(datetime, react_pwr))
